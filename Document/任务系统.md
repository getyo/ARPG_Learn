...
6. `QuestTargetCondition` 提供两个进度读写虚接口：`GetSaveProgress()`（返回 `>=0` 表示可持久化的进度，`-1` 表示无需持久化）与 `ApplySaveProgress(int32)`；目前由 `KillCondition` 重写，其 `_RequiredCount`（需求击杀数）与 `_RemainingCount`（剩余击杀数，运行时递减）分开存储。
7. `QuestSubSystem` 提供 `GetAllQuestInfos()`，供存档侧遍历所有任务/阶段/条件读写进度；`_QuestInfos` 声明为 `UPROPERTY`，以使其中的条件对象受 GC 追踪。
...
6. 关于存档：`AThirdPersonPlayerController`（蓝图`BP_PlayerControler`）实现了`IPlayerSaveInterface`（`SavePlayerData`/`LoadPlayerData`）。存档会保存玩家的背包、当前装备与**任务进度**（当前任务及其阶段、聚焦任务、各条件的剩余数，见`FQuestSaveData`）；读档时直接覆盖`ActiveQuests/FocusedQuest`并回写条件计数，中途进度（如“击杀 3/5”）会继续累计。具体设计见**存档系统.md**。
