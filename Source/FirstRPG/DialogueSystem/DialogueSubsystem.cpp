...
bool UDialogueSubsystem::StartDialogue(const FGameplayTag& QuestTag, int Stage, AThirdPersonPlayerController* PlayerController)
{
	...
	{
		CPP_LOG(Warning, FString::Printf(TEXT("Cannot Find Quest's Dialogue: QuestID : %s"),
			*QuestTag.ToString()));
		return false;
	}
	...
	{
		//该阶段本来就可能没配对话（如纯战斗阶段），按 Verbose 输出，不刷红字
		CPP_LOG(Verbose, FString::Printf(TEXT("Cannot Find QuestStage's Dialogue: QuestID : %s , Stage : %d"),
...
bool UDialogueSubsystem::EndDialogue()
{
	...
	auto CharacterManager = GI->GetSubsystem<UCharacterManagerSubsystem>();
	if (!CharacterManager) return false;
	// 取当前阶段的对话资产：用 Find 访问，避免下标访问在缺失时往 map 里插空指针
	UDialogueDataAsset* StageAsset = nullptr;
	if (const FInQuestTargetDialogMap* DialogMapPtr = DialogueAssetMap.Find(CurrentQuest))
	{
		StageAsset = DialogMapPtr->Map.FindRef(CurrentStage);
	}
	if (!StageAsset)
	{
		CPP_LOG(Warning, FString::Printf(TEXT("EndDialogue: Cannot Find QuestStage's Dialogue: QuestID : %s , Stage : %d"),
			*CurrentQuest.ToString(), CurrentStage));
		return false;
	}
	TSet<FGameplayTag> Characters;
	//把所有相关的Character的InDialogye设置为true
	for (auto Elem : StageAsset->DialogueLines)
	{
		Characters.Add(Elem.SpeakerTag);
	}
	for (auto CharacterTag : Characters)
	...
	//广播对话完成
	//标签无效说明为日常对话，不用广播
	if (StageAsset->RelativeQuestTag.IsValid())
	{
