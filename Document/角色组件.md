...
到`Character Status`组件细节面板搜索`Character Status`标签进行设置即可，如下：
...
## 2026-09-13 更新（与存档的交互）

- `UEquipmentComponent` 在存档链路中改为**可选依赖**：
  - `AGeneralCharacter::SaveCharacterData_Implementation` 找不到该组件时只跳过"装备状态"部分，其余字段（class / 位置 / IsStatic / Name）照写；
  - `LoadCharacterData_Implementation` 找不到该组件时安静跳过装备恢复。
- 原因：马、象、动物这类角色没有装备组件，旧实现会在这里整段 `return`，导致它们的存档数据全是默认值（读档后被误当成"动态角色"重建而失败）。
- 未改动：`UEquipmentComponent` 自身的逻辑与被它管理的装备实例（`UEquipmentInstance`）没有变化。
