...
void AThirdPersonPlayerController::SavePlayerData_Implementation(FPlayerSaveData& Data)
{
	...
	Data.QuestData.ConditionProgress.Reset();
	if (!QuestSubsystem)
	{
	...
	if (QuestSubsystem)
	{
		for (auto& QuestPair : QuestSubsystem->GetAllQuestInfos())
		{
			FS_QuestStagesProgress StagesProgress;
			for (auto& Target : QuestPair.Value.Targets)
			{
				FS_QuestConditionProgress Progress;
				Progress.StageInt = Target.StageInt;
				Progress.ConditionsRemaining.Reserve(Target.Conditions.Num());
				for (auto* Condition : Target.Conditions)
				{
					Progress.ConditionsRemaining.Add(IsValid(Condition) ? Condition->GetSaveProgress() : -1);
				}
				StagesProgress.Stages.Add(Progress);
			}
			Data.QuestData.ConditionProgress.Add(QuestPair.Key, StagesProgress);
		}
	}
	//剩下的保存交给蓝图处理
}
...
void AThirdPersonPlayerController::LoadPlayerData_Implementation(const FPlayerSaveData& Data)
{
	...
	// ===== 任务系统读档 =====
	ActiveQuests = Data.QuestData.ActiveQuests;
	FocusedQuest = Data.QuestData.FocusedQuest;
	if (!QuestSubsystem)
	{
		UWorld* World = GetWorld();
		if (UGameInstance* GI = World ? World->GetGameInstance() : nullptr)
	...
	if (QuestSubsystem)
	{
		for (auto& ProgressPair : Data.QuestData.ConditionProgress)
		{
			FS_QuestInfo* QuestPtr = QuestSubsystem->GetAllQuestInfos().Find(ProgressPair.Key);
			if (!QuestPtr) continue;
			for (auto& Progress : ProgressPair.Value.Stages)
			{
				for (auto& Target : QuestPtr->Targets)
				{
					if (Target.StageInt != Progress.StageInt) continue;
					const int32 Count = FMath::Min(Target.Conditions.Num(), Progress.ConditionsRemaining.Num());
					for (int32 i = 0; i < Count; ++i)
					{
						if (IsValid(Target.Conditions[i]) && Progress.ConditionsRemaining[i] >= 0)
						{
							Target.Conditions[i]->ApplySaveProgress(Progress.ConditionsRemaining[i]);
						}
					}
				}
			}
