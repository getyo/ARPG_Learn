...
void UEquipmentComponent::SetArmedState(E_ArmedState NewState)
{
	if (ArmedState != NewState)
	{
		ArmedState = NewState;
		OnArmedStateChanged.Broadcast(NewState);
	}
}

//手占用规则：一把武器只能占一套手位。双手武器（弓）在手上时，盾牌必须收回装备位，
//否则左手会被“弓”和“盾”同时占用，出现“掏弓还拿盾”的渲染状态
void UEquipmentComponent::EnforceHandRules()
{
	if (ArmedState == E_ArmedState::RangeArmed && bShieldArmed && CurEquipmentRef.Shield)
	{
...
bool UEquipmentComponent::SetEquippedRangeWeapon(UEquipmentInstance* Instance)
{
	...
	if (Instance->GetStMesh())
	{
		...
		if (CurEquipmentRef.RangeWeapon)
		{	
			//互斥：装备双手武器时，绝不能把盾牌留在手上
			if (Instance->IsTwoHanded() && bShieldArmed)
			{
				SheatheShield();
			}
			EnforceHandRules();
...
{
	if (!CurEquipmentRef.RangeWeapon) return;
	if (GetOwner()->ActorHasTag("Player"))
...
bool UEquipmentComponent::DrawShield()
{
	...
	//手占用规则：若正持有双手武器（弓），先把它收回装备位，避免“弓+盾”同时占手
	if (ArmedState == E_ArmedState::RangeArmed && UsingWeapon)
	{
		DestroyUsingWeapon();
		SetArmedState(E_ArmedState::None);
	}
	...
	bShieldArmed = (CurEquipmentRef.Shield != nullptr);
	return bShieldArmed;
}
	...
	case E_EquipmentCategory::MeleeWeapon:
		{
			...
			SetArmedState(E_ArmedState::MeleeArmed);
			EnforceHandRules();
			return true;
		}
	case E_EquipmentCategory::RangeWeapon:
		{
			...
			SetArmedState(E_ArmedState::RangeArmed);
			EnforceHandRules();
			return true;
		}
	default:break;
