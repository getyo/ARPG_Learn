...
protected:
	...
	UPROPERTY(VisibleAnywhere,BlueprintReadOnly,Category = "EquipmentParas")
	FString ArmedSocketName;
	//双手武器标记：可在蓝图/数据层覆盖；默认远程武器按双手处理，近战武器按单手处理
	UPROPERTY(EditAnywhere,BlueprintReadWrite,Category = "EquipmentParas")
...
public:
	...
	//是否占用双手（占用双手的武器武装时，盾牌必须收回装备位）
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Equipment | Instance")
	bool IsTwoHanded() const { return bTwoHanded || EquipmentCategory == E_EquipmentCategory::RangeWeapon; }
