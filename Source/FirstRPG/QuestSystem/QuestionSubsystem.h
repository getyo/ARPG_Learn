...
public:
	...
	// 供存档系统读写各任务/目标的运行时条件进度
	TMap<FGameplayTag, FS_QuestInfo>& GetAllQuestInfos() { return _QuestInfos; }
...
private:
	// 必须是 UPROPERTY：FS_QuestTarget::Conditions 里的 UQuestTargetCondition* 靠这条链才能被 GC 追踪，
	// 否则会被回收成悬空指针（存档时解引用会直接崩）
	UPROPERTY()
