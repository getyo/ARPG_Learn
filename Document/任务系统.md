...
classDiagram 
	...
	class QuestSubSystem{
		...
		+ FS_QuestTarget GetQuestTarget(const FGameplayTag& QuestID,int Stage)
		+ FS_QuestTarget GetNextQuestTarget(const FGameplayTag& QuestID,int Stage)
...
classDiagram
	...
	- static const TMap _E2Class; //从Tag到条件子类型的映射，用于工厂函数
	}
...
FS_PlayerQuestHandler *-- BP_PlayerControler
BP_AICharacter --o QuestComponent
BP_AICharacter <|-- BP_QuestionGiver
BP_QuestionGiver --o QuestComponent
BP_ThirdPersonCharacter --* BP_PlayerControler
FS_ActorHoldTarget *-- FS_ActorHoldQuestTargets
AThirdPersonPlayerController --|> BP_PlayerControler
QuestComponent --* FS_ActorHoldQuestTargets 
QuestComponent --o AThirdPersonPlayerController
...
+ bool GotoNextStage(FGameplayTag QuestTag) 	//当前任务结束时调用QuestFinished
+ bool IsQuestActive(FGameplayTag QuestTag)
+ bool IsQuestFinished(FGameplayTag QuestTag)
+ bool ActiveQuestEmpty()
+ bool HasFocusedQuest()
+ void AddActiveQuest(FGameplayTag QuestTag)
+ void RemoveActiveQuest(FGameplayTag QuestTag)
+ TMap GetAllActiveQuest()
+ FS_PlayerHoldTarget GetQuestHoldTarget(FGameplayTag QuestTag)
+ int GetQuestStageInt(FGameplayTag QuestTag)
+ bool IsQuestFocused(FGameplayTag QuestTag)
+ bool SetFocusedQuest(FGameplayTag QuestTag)
+ void QuestFinished(FGameplayTag QuestTag)
}
class BP_QuestionGiver{
...
6. 关于存档：`AThirdPersonPlayerController`（蓝图`BP_PlayerControler`）实现了`IPlayerSaveInterface`（`SavePlayerData`/`LoadPlayerData`），目前存档会保存玩家的背包与当前装备，**任务进度暂未纳入存档**。具体设计见**存档系统.md**。
...
#### 3.2.1 到`Content\QusetionSystem`目录下，打开`DB_Quest`，添加你要编辑的任务信息，当你添加一行以后基本的面板如下：
...
2. 到`Content\QusetionSystem`目录下，打开`DB_GameplayTag`，添加`Quest.Action.YourAction`，注意行名和行内标签必须一致
...
点击添加好的`Quest`，在下方细节面板可以看到`Quest`标签下的三个关键变量：

<img src="image-20260424113121346.png" alt="image-20260424113121346" style="zoom:50%;" />

这两个变量就是Actor持有的所有任务信息，这里我已经设置好了，你的面板应该是`None`和`0个贴图元素`。接下来我说明下这三个变量如何设置
...
3. `Focused Quest Tag`：现在角色聚焦于哪个任务，主要用于触发对话，因为一个角色可能有多个相关对话，你必须选出一个对话来，至于Kill条件这种就不看角色的聚焦任务，因为只要你杀死了强盗，那么所有要求杀死强盗的任务都应该被完成。
...
## 2026-09-13 更新（任务进度可存档）

### 1. 击杀条件的计数拆分
- `UKillCondition`：把 `_RequiredCount`（需求数，配置值）与 `_RemainingCount`（剩余数，运行时递减）拆开；
  `ConditionPassed` 改为递减剩余数，并补上原先缺失的 `Lock.Unlock()`。

### 2. 条件进度读写接口
- `UQuestTargetCondition` 新增两个虚接口：`int32 GetSaveProgress() const`（默认返回 -1，表示无需持久化）与 `void ApplySaveProgress(int32)`。

### 3. 暴露任务定义给存档
- `UQuestionSubsystem::GetAllQuestInfos()`：供存档侧遍历所有任务/阶段/条件，读写进度。
- `_QuestInfos` 加 `UPROPERTY`，否则其中的条件对象会被 GC 回收（详见《存档系统》第 6 条）。

### 4. 任务进度存档行为
- 存档：玩家任务进度（当前阶段 / 聚焦任务 / 各条件剩余数）随玩家数据一起入库。
- 读档：直接恢复 `ActiveQuests / FocusedQuest`，并回写条件计数；中途进度（例如"击杀 3/5"）会继续累计，不会退回阶段开头。

### 5. 日志
- 找不到某阶段对话时的红色报错已降级为 Verbose（战斗阶段本来就没有对话资产），详见《对话系统》。
